...
PHYSICAL_REGISTER_ID RegisterAllocator::rename_dest_register(int16_t reg, champsim::program_ordered<ooo_model_instr>::id_type producer_id)
{
  ...
  physical_register_file.at(phys_reg) = {(uint16_t)reg, producer_id, false, true};
...
bool RegisterAllocator::isBusy(PHYSICAL_REGISTER_ID physreg) const { return physical_register_file.at(physreg).busy; }

bool RegisterAllocator::isAllocated(PHYSICAL_REGISTER_ID archreg) const { return frontend_RAT[archreg] != -1; }

PHYSICAL_REGISTER_ID RegisterAllocator::frontend_RAT_lookup(int16_t archreg) const { return frontend_RAT[archreg]; }

uint16_t RegisterAllocator::get_arch_reg(PHYSICAL_REGISTER_ID physreg) const { return physical_register_file.at(physreg).arch_reg_index; }
