...
long DRAM_CHANNEL::finish_dbus_request()
{
  ...
  if (active_request != std::end(bank_request) && active_request->ready_time <= current_time) {
    if (active_request->pkt->value().tile_burst) {
         // Burst Mode: Respond 16 times
         uint64_t base_addr_val = active_request->pkt->value().address.to<uint64_t>() - (active_request->pkt->value().tile_subidx * 64);
         
         for (int i=0; i<16; ++i) {
             champsim::address slot_addr{base_addr_val + (i * 64)};
             response_type response{slot_addr, active_request->pkt->value().v_address, active_request->pkt->value().data,
...
long DRAM_CHANNEL::populate_dbus()
{
  ...
  if (iter_next_process->valid && iter_next_process->ready_time <= current_time) {
    if (active_request == std::end(bank_request) && dbus_cycle_available <= current_time) {
      ...
      // Tile burst transfers 16 cache lines (1024B); normal transfers 1 (64B).
      auto bus_time = active_request->pkt->value().tile_burst
      ...
      if (bankgroup_ready_time > current_time)
        active_request->ready_time = bankgroup_ready_time + bus_time;
      else
        active_request->ready_time = current_time + bus_time;

      // set when bankgroup dbus will be next ready
      bankgroup_readytime[op_bankgroup] = current_time + bus_time + DRAM_DBUS_BANKGROUP_STALL;
      ...
      if (iter_next_process->row_buffer_hit) {
        if (write_mode) {
          ...
          if (iter_next_process->pkt->value().is_amx_tileload)
            ++sim_stats.RQ_TILELOAD_ROW_BUFFER_HIT;
        }
      } else if (write_mode) {
        ...
        if (iter_next_process->pkt->value().is_amx_tileload)
          ++sim_stats.RQ_TILELOAD_ROW_BUFFER_MISS;
      }
...
long DRAM_CHANNEL::service_packet(DRAM_CHANNEL::queue_type::iterator pkt)
{
  ...
  if (pkt->has_value() && pkt->value().ready_time <= current_time) {
    ...
    if (!bank_request[op_idx].valid && !bank_request[op_idx].under_refresh) {
      ...
      if (pkt->value().tile_burst) {
        // Tile burst: 16 sequential column reads from the same row.
        // First line: row activation (if miss) + tCAS. Remaining 15: tCAS each.
        // Total bank occupancy = row_charge_delay (if miss) + 16 * tCAS
        ready = current_time + tCAS * 16 + (row_buffer_hit ? champsim::chrono::clock::duration{} : row_charge_delay);
      } else {
        ready = current_time + tCAS + (row_buffer_hit ? champsim::chrono::clock::duration{} : row_charge_delay);
      }

      bank_request[op_idx] = {true,  row_buffer_hit,        false,
                              false, std::optional{op_row}, ready,
                              pkt};
...
void MEMORY_CONTROLLER::initiate_requests()
{
  ...
  // Fix: use stable_partition instead of get_span_p to avoid head-of-line blocking.
  // get_span_p stops at the first add_rq failure, blocking requests to other DRAM
  // channels when one channel's RQ is full. stable_partition tries ALL requests,
  // keeping failed ones in FIFO order for retry next cycle.
  for (auto* ul : queues) {
    for (auto q : {std::ref(ul->RQ), std::ref(ul->PQ)}) {
      auto& queue = q.get();
      auto kept = std::stable_partition(queue.begin(), queue.end(),
          [ul, this](const auto& pkt) { return !this->add_rq(pkt, ul); });
      queue.erase(kept, queue.end());
    }

    // Initiate write requests
    auto& wq = ul->WQ;
    auto wq_kept = std::stable_partition(wq.begin(), wq.end(),
        [this](const auto& pkt) { return !this->add_wq(pkt); });
    wq.erase(wq_kept, wq.end());
  }
}

DRAM_CHANNEL::request_type::request_type(const typename champsim::channel::request_type& req)
    : pf_metadata(req.pf_metadata), tile_burst(req.tile_burst), tile_subidx(req.tile_subidx), is_amx_tileload(req.is_amx_tileload), address(req.address), v_address(req.address), data(req.data), instr_depend_on_me(req.instr_depend_on_me)
{
  asid[0] = req.asid[0];
  asid[1] = req.asid[1];
}
