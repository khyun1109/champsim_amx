...
// Like do_collision_for_merge but also refuses to merge into a tile-protected target
template <typename Iter>
bool do_collision_for_merge_no_tile(Iter begin, Iter end, champsim::channel::request_type& packet, champsim::data::bits shamt)
{
  using request_type = champsim::channel::request_type;
  if (auto found = std::find_if(begin, end, [match = packet.address.slice_upper(shamt), shamt](const auto& x) {
        return x.address.slice_upper(shamt) == match;
      }); found != end && packet.is_translated == found->is_translated) {
    // Refuse merge if target is also tile-protected
    if (found->tile_no_rq_merge) return false;
    // Normal merge
    found->response_requested |= packet.response_requested;
    auto instr_copy = std::move(found->instr_depend_on_me);
    std::set_union(std::begin(instr_copy), std::end(instr_copy),
                   std::begin(packet.instr_depend_on_me), std::end(packet.instr_depend_on_me),
