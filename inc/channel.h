...
class channel
{
  struct request {
    ...
    // AMX Tile Grouping Support (Sidecar State Semantics)
    // These fields are AUXILIARY metadata carried by ordinary line requests.
    // They do NOT replace line-address-based semantics.
    bool is_amx_tileload = false;
    bool tile_burst = false; // Legacy flag (can be deprecated)
    uint64_t tile_group_id = 0;
    uint8_t tile_subidx = 0;
    uint8_t tile_num_children = 0;  // Total children in this tile parent
    bool tile_is_store = false;     // true for TILESTORED, false for TILELOADD/DT1
    bool tile_no_rq_merge = false;  // Prevent RQ merge for tile LQ parent children
    bool skip_l1_fill = false;      // L2-only SW prefetch: skip L1D fill on response
