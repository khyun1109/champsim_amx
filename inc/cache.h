...
class CACHE : public champsim::operable
{
  ...
  struct tag_lookup_type {
    ...
    // AMX Tile metadata (auxiliary, does NOT replace line address)
    bool is_amx_tileload = false;
    bool tile_burst = false; // Legacy flag
    uint64_t tile_group_id = 0;
    uint8_t tile_subidx = 0; // Index of the cache line within the tile (0-15)
    uint8_t tile_num_children = 0; // Total children in parent
    bool tile_is_store = false;
    uint8_t tile_dest_tmm = 0;   // destination TMM register (199-202)
    ...
    explicit tag_lookup_type(request_type req) : tag_lookup_type(req, false, req.skip_l1_fill) {}
    tag_lookup_type(const request_type& req, bool local_pref, bool skip);
  };

public:
  // ============================================================================
  // Queue-Local Tile Sidecar State (Passive Completion Tracking)
  // ============================================================================
  // Lightweight sidecar metadata indexed by tile_group_id.
  // Does NOT own or schedule child requests — children flow as normal line
  // requests through RQ/MSHR/LFB.  The sidecar only tracks how many children
  // have been observed and how many have completed (hit or fill).
  //
  // Key invariants:
  // - Cache data path remains line-based (no tile-sized blocks)
  // - Line lookups/fills/replacements unchanged
  // - No throttling or active scheduling — purely observational
  // ============================================================================
  struct tile_sidecar_state {
    bool valid = false;
    uint64_t tile_group_id = 0;
    uint8_t num_children = 0;           // Total children in this tile (from tile_num_children)
    uint8_t observed_children = 0;      // How many distinct children we have seen so far
    uint8_t completed_count = 0;        // How many children completed (hit + fill)
    uint16_t completed_mask = 0;        // bit i=1 → child i completed
    uint16_t hit_mask = 0;             // bit i=1 → child i hit in cache
    uint16_t observed_mask = 0;         // bit i=1 → child i has been seen
    uint64_t first_seen_cycle = 0;
    uint64_t completion_cycle = 0;

    bool is_complete() const { return num_children > 0 && completed_count >= num_children; }
  };
  ...
  struct mshr_type {
    ...
    bool skip_fill = false;   // When true, don't install line in this cache on fill
    uint8_t serve_level = 0;  // 0=L2_HIT, 1=LLC_HIT, 2=DRAM (set by finish_packet)
    ...
    bool is_amx_tileload = false;  // true if this MSHR entry was created by a tile load child

    // --- Tile Attachment Support (line-level MSHR entries) ---
    // For normal (non-tile) MSHR entries that happen to serve a tile child,
    // record which tile group the line belongs to.
    struct tile_attachment {
      uint64_t tile_group_id;
      uint8_t tile_child_idx;
    };
    std::vector<tile_attachment> tile_attachments{};

    // --- Tile-Level MSHR Entry (OptAMX: 1 entry per tile) ---
    // When is_tile_entry=true, this MSHR entry represents an entire tile.
    // All 16 children share this single MSHR slot.
    // Children's individual fills are tracked via tile_children.
    bool is_tile_entry = false;
    uint64_t tile_group_id = 0;
    uint8_t tile_num_children = 0;

    struct tile_child_entry {
      champsim::address address{};
      champsim::address v_address{};
      champsim::address ip{};
      champsim::address data{};
      ...
      access_type type = access_type::LOAD;
      bool prefetch_from_this = false;
      bool skip_fill = false;         // B tile bypass: don't install in L1D
      uint8_t child_idx = 0;
      uint8_t serve_level = 0;  // 0=L2_HIT, 1=LLC_HIT, 2=DRAM

      std::vector<uint64_t> instr_depend_on_me{};
      std::vector<std::deque<response_type>*> to_return{};

      // Fill tracking
      bool sent_to_lower = false;       // request sent to L2C
      bool fill_received = false;       // data returned from lower level
      bool fill_installed = false;      // cache line installed
      champsim::chrono::clock::time_point fill_ready_at{};  // when fill can be installed
    };
    std::vector<tile_child_entry> tile_children{};
...
private:
  ...
  // Tile-level prefetch queue: 1 entry = 1 tile (16 CL).
  // Processed with leftover demand bandwidth (like Berti/standard PQ model).
  // 1 bandwidth unit processes 1 tile entry (creates tile MSHR + issues 16 children).
  static constexpr std::size_t TILE_PQ_SIZE = 16;
  struct tile_pq_entry {
    uint64_t tile_group_id;
    std::vector<champsim::address> addrs;
    bool fill_this_level;
  };
  std::deque<tile_pq_entry> tile_PQ{};

public:
  std::vector<channel_type*> upper_levels;
  channel_type* lower_level;
  channel_type* lower_translate;
  class VirtualMemory* vmem = nullptr;  // for SB direct VA→PA translation

  uint32_t cpu = 0;
  std::string NAME;
  uint32_t NUM_SET, NUM_WAY, MSHR_SIZE, LFB_SIZE;
  std::size_t PQ_SIZE;
  ...
  // Tile metadata bridge: populated before prefetcher_cache_operate() so the
  // prefetcher can read tile info for the current access via intern_->current_tile_info.
  struct tile_access_info {
    bool is_amx_tileload = false;
    uint64_t tile_group_id = 0;
    uint64_t instr_id = 0;
  ...
  tile_access_info current_tile_info{};

  // Tile-aware replacement hint: set before calling impl_replacement_cache_fill
  // so the replacement policy can insert tile data at lower LRU priority.
  bool current_fill_is_tile = false;

  // ============================================================================
  // SW Prefetch Lifecycle Tracker (observation only, L1D only)
  // Tracks: prefetch fill → demand access timing, eviction detection
  // ============================================================================
  struct pf_tracker_stats {
    uint64_t pf_requests = 0;        // total SW prefetch requests seen
    uint64_t pf_l1_hit = 0;          // prefetch hit L1 (redundant)
    uint64_t pf_l1_miss = 0;         // prefetch missed L1 (will fetch from lower)
    uint64_t pf_fills = 0;           // prefetch data filled into L1
    uint64_t demand_hit_pf_line = 0; // demand L1 hit on a prefetch-filled line
    uint64_t demand_miss_pf_evicted = 0; // demand L1 miss, prefetch filled but evicted
    uint64_t demand_no_pf = 0;       // demand for address with no prior prefetch
    uint64_t demand_total_hits = 0;  // all demand tileload L1 hits
    uint64_t demand_total_misses = 0;// all demand tileload L1 misses
    uint64_t pf_evicted_before_use = 0; // prefetch line evicted before any demand used it
    // Timing
    uint64_t total_pf_to_demand_gap = 0; // sum of (demand_cycle - pf_fill_cycle) for hits
    uint64_t pf_to_demand_count = 0;
    uint64_t total_pf_residency = 0;    // sum of (evict_cycle - pf_fill_cycle) for evicted-before-use
    uint64_t pf_residency_count = 0;
    uint64_t pf_fill_latency_total = 0;  // sum of (fill_cycle - mshr_enqueue_cycle)
    uint64_t pf_fill_latency_count = 0;
  };
  pf_tracker_stats pf_stats{};

  // Map: block_address → fill_cycle (for lines brought in by SW prefetch)
  std::unordered_map<uint64_t, uint64_t> pf_filled_lines{};

  std::deque<mshr_type> MSHR;
  std::deque<mshr_type> LFB;  // DEPRECATED: unused in unified LFB model (MSHR serves as unified pool)
  std::deque<mshr_type> inflight_writes;

  // ============================================================================
  // Queue-Local Tile Sidecar Table (Passive Completion Tracking)
  // ============================================================================
  // Lightweight sidecar state indexed by tile_group_id.
  // Children flow as normal line requests; the sidecar only observes.
  // ============================================================================
  std::unordered_map<uint64_t, tile_sidecar_state> tile_sidecar{};

  // Sidecar helper methods
  tile_sidecar_state* find_or_alloc_sidecar(uint64_t tile_group_id, uint8_t num_children);
  tile_sidecar_state* find_sidecar(uint64_t tile_group_id);
  void sidecar_mark_child_complete(uint64_t tile_group_id, uint8_t child_idx);
  ...
  [[nodiscard]] bool has_mshr_entry_for(champsim::address addr) const;
  ...
  [[nodiscard]] std::size_t get_lfb_occupancy() const;
  [[nodiscard]] std::size_t get_lfb_size() const;
  [[nodiscard]] std::size_t get_demand_mshr_occupancy() const;
  ...
  // Tile-coalesced prefetch: issues a prefetch with tile metadata so it can
  // coalesce into a single tile MSHR entry (same as demand tileloads with sidecar).
  bool prefetch_tile_line(champsim::address pf_addr, bool fill_this_level,
                          uint64_t tile_group_id, uint8_t tile_subidx, uint8_t tile_num_children);

  // Tile-level prefetch: bypasses PQ, directly creates a tile MSHR entry for
  // all CL addresses. 1 call = 1 MSHR entry = N lower-level requests.
  // Returns false if MSHR is full.
  bool prefetch_tile(const std::vector<champsim::address>& addrs,
...
#include "module_decl.inc"
  ...
  explicit CACHE(champsim::cache_builder<champsim::cache_builder_module_type_holder<Ps...>, champsim::cache_builder_module_type_holder<Rs...>> b)
      : champsim::operable(b.m_clock_period), upper_levels(b.m_uls), lower_level(b.m_ll), lower_translate(b.m_lt), NAME(b.m_name), NUM_SET(b.get_num_sets()),
        NUM_WAY(b.get_num_ways()), MSHR_SIZE(b.get_num_mshrs()), LFB_SIZE(b.get_num_lfbs()), PQ_SIZE(b.m_pq_size),
        HIT_LATENCY(b.get_hit_latency() * b.m_clock_period),
