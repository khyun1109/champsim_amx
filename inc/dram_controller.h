...
public:
  // Tileload PA-level remap for row buffer hit optimization
  bool remap_tileload_pa_ = false;
private:
  std::unordered_map<uint64_t, uint64_t> tile_pa_remap_;
  uint64_t tile_pa_next_col_ = 0;
  champsim::address remap_tileload_pa(champsim::address addr);
