...
struct cache_builder_base {
  ...
  std::optional<uint32_t> m_lfb_size{};
...
public:
  ...
  /**
   * Specify the number of LFBs.
   * If this is not specified, it will default to the number of MSHRs.
   */
  self_type& lfb_size(uint32_t lfb_size_);
...
template <typename P, typename R>
auto champsim::cache_builder<P, R>::get_num_lfbs() const -> uint32_t
{
  return std::max(m_lfb_size.value_or(get_num_mshrs()), 1u);
}
...
template <typename P, typename R>
auto champsim::cache_builder<P, R>::lfb_size(uint32_t lfb_size_) -> self_type&
{
  m_lfb_size = lfb_size_;
  return *this;
}
