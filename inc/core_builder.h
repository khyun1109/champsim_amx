...
public:
  ...
  /**
   * Specify a pointer to the L1D cache. Used for cache-level stall breakdowns.
   */
  self_type& l1d(CACHE* l1d_);

  /**
   * Specify a pointer to the L2 cache. Used for cache-level stall breakdowns.
   */
  self_type& l2c(CACHE* l2c_);

  /**
   * Specify a pointer to the LLC. Used for cache-level stall breakdowns.
   */
  self_type& llc(CACHE* llc_);
...
template <typename B, typename T>
auto champsim::core_builder<B, T>::l2c(CACHE* l2c_) -> self_type&
{
  m_l2c = l2c_;
  return *this;
}
...
template <typename B, typename T>
auto champsim::core_builder<B, T>::llc(CACHE* llc_) -> self_type&
{
  m_llc = llc_;
  return *this;
}
