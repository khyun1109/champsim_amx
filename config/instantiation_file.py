...
def get_cpu_builder(cpu, caches, ul_pairs):
    ...
    def cache_index_optional(name):
        for idx, cache in enumerate(caches):
            if cache.get('name') == name:
                return idx
        return None

    def cache_by_name(name):
        return next((cache for cache in caches if cache.get('name') == name), None)

    l1d_cache = cache_by_name(cpu.get('L1D'))
    l2_cache = cache_by_name(l1d_cache.get('lower_level')) if l1d_cache else None
    llc_cache = cache_by_name(l2_cache.get('lower_level')) if l2_cache else None
    ...
    if l2_cache is not None:
        l2_index = cache_index_optional(l2_cache.get('name'))
        if l2_index is not None:
            local_params['^l2c_ptr'] = f'(*std::next(std::begin(caches), {l2_index}))'
            required_parts.append('.l2c(&{^l2c_ptr})')
    if llc_cache is not None:
        llc_index = cache_index_optional(llc_cache.get('name'))
        if llc_index is not None:
            local_params['^llc_ptr'] = f'(*std::next(std::begin(caches), {llc_index}))'
            required_parts.append('.llc(&{^llc_ptr})')
    if 'frequency' in cpu:
